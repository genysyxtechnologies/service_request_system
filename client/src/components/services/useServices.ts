...
export const useServices = (token: string) => {
    ...
    const fetchDashBoard = async () => {
        setLoading(true);
        try {
            const response = await service.getServices(ENDPOINTS.SUPER_ADMIN_DASHBOARD);
            if (response as AxiosData) {
    ...
    return { createService, services, loading, setLoading, error, setServices, fetchServices, fetchDashBoard, deleteService, serviceId, setServiceId, updateService, dashboad, setDashBoard, isServerError }
}